...
for session in $(ls -d sess*); do #$(ls -d sess*)
	...
	ln -s ${WD}/${session}/preproc/motion_params/MP_run01.txt ${WD}/${session}/preproc/localWMreg/${session}_MP_run01.1D
	ln -s ${WD}/${session}/preproc/motion_params/MP_run02.txt ${WD}/${session}/preproc/localWMreg/${session}_MP_run02.1D
	ln -s ${WD}/${session}/preproc/motion_params/MP_run03.txt ${WD}/${session}/preproc/localWMreg/${session}_MP_run03.1D
	ln -s ${WD}/${session}/preproc/motion_params/MP_run04.txt ${WD}/${session}/preproc/localWMreg/${session}_MP_run04.1D
	...
	for run in run01 run02 run03 run04; do
		...
		#${WD}/${session}/preproc/localWMreg/${session}_preproc_${run}.nii.gz > ${WD}/${session}/preproc/localWMreg/${session}_GS_${run}.1D
