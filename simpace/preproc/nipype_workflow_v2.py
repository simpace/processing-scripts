...
# 1st arg: 01 (sub01); 2nd argument: 01 (sess01; optional, otherwise does all sessions)
...
# setup directories
# ASSUMES DIRECTORY STRUCTURE: data_dir/sub01/sess01/... and data_dir/sub01/anatomical/nifits/*.nii
...
# Set up anatomical proc to run if doesn't exist
...
if TO_run_anatomical:
    ...
    # Co-registration of template to subject brain
    ...
    rename_files = pe.Node(interface=Function(input_names=['files', 'dest_dir'], output_names='out', function=mv_files),
                           name='rename_files')
    rename_files.inputs.dest_dir = pjoin(anat_proc_dir, 'coreg_mni_to_native')
...
for isess, sess_dir in enumerate(sessions):
    ...
    stc.inputs.slice_order = range(32, 0, -1)  # slices are descending
    ...
    motion_params.inputs.parse_string = 'rp_.*_.*_(?P<run_name>\w*)_.*.txt'
    ...
    motion_params_movefiles.inputs.dest_dir = pjoin(sess_dir, workflow_name_func, 'motion_params')
    ...
    workflow_func.run()
    os.system(pjoin('gzip ' + workflow_dir, 'stc', 'a*.nii'))
    os.system(pjoin('gzip ' + workflow_dir, 'realign', 'ra*.nii'))
